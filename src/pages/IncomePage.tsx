...
export default function IncomePage() {
  ...
  const categories = ['Semua', 'Penjualan', 'Investasi', 'Bonus', 'Lainnya'];
    ...
    <div className="space-y-12">
      ...
      <header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-black tracking-tighter lg:text-5xl">Pemasukan</h1>
        </div>
        <button 
        ...
        {loading ? (
          ...
          filteredTransactions.map((t) => (
            ...
            >
              <div className="flex items-center gap-6">
                <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-emerald-50 text-emerald-600 transition-transform duration-500 group-hover:scale-110">
                  <TrendingUp size={28} />
                </div>
                <div>
          ...
          <div className="py-32 text-center rounded-5xl border-4 border-dashed border-black/[0.03]">
            <div className="mx-auto h-20 w-20 rounded-full bg-black/[0.02] flex items-center justify-center mb-6">
              <TrendingUp size={40} className="text-black/10" />
            </div>
            <p className="text-sm font-black text-black/20 uppercase tracking-[0.3em]">No income records found</p>
          </div>
        )}
      </div>
